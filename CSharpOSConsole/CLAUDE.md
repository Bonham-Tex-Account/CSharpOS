...
| **`Snake`** | game (§3) | the snake game. ~240 lines. W=8×H=8 grid, life-countdown body, INPOLL arrows, LCG food, whole-grid OUTS/frame. Grep internal labels: `main`, `move`, `nogrow`, `dead`, `place_food`, `render`, `border`. DATA: state @2048, GRID @2112, REND @2624. Local C# helpers `Ld`/`StR`/`StI`/`EmitR11`. **Gotcha (fixed):** `StR(addr, src)` loads `addr` into **EAX first**, so `StR(cell, EAX)` stores the address, not the value — hold a value in a non-EAX reg (e.g. R11) before `StR`. This bit `place_food` (pinned the food + hung on eat) |
