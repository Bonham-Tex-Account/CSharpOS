...
| 2026-07-07 | Root-caused the FSYS-exec "alternation" flake (harness artifact, not an OS bug) + §4.1 AsmTable mnemonic/register tables | ~35K (inline) | Flake: `os.HasProcesses` goes false mid-teardown (interrupts masked) → `while(HasProcesses)`+`LoadProcess` injects into a parked machine → alternates. Fix = `Test.RunUntilIdle` drains to clean idle; regression `FsExecChainTests` (3). Diagnosed with a temp repro + event/state tracing (0 user-instrs/0 os-routines on failing runs was the tell). §4.1: `CSharpOS/Assembler/AsmTable.cs` (OperandShape enum, mnemonic+register tables, ISA word-per-char serialization) + `AsmTableTests` (27). 708 tests. |
...
| 2026-07-11 | Fix "snake (any foreground job) doesn't visualize in the dashboard" — the Screen panel now follows the OS-designated foreground process | ~30K (inline) | Root cause: **two divorced notions of foreground** — `hw` active process (set by the shell's `SETFOCUS` to a fg child; routes the keyboard, so snake *received* keys) vs the dashboard's `model.FocusedProcess` (the only thing the Screen panel reads). `EnsureFocus` only re-focused when the *current* focus died, so with the shell (P0) staying live in `WAIT`, the panel stayed pinned to the shell and the child's (P1) grid never showed. Fix (`SpectreDashboard.EnsureFocus`): adopt `hw.GetActiveProcess()` when it diverges from the value the dashboard last set (= an OS `SETFOCUS`); Tab installs a `manualFocus` override that sticks until its process dies / the user Tabs back / the OS moves the foreground. New fields `lastSeenActive`/`osForeground`/`manualFocus`; `SetFocus` now also syncs `lastSeenActive`. Regression `SpectreDashboardTests.Screen_FollowsForegroundChild_WhenParentHandsOffWithSetFocus` (FORK→SETFOCUS(child)→WAIT, child OUTS a canvas frame; verified red without the fix). 722 tests. Snake/game/output path itself was already correct (SnakeTests prove the frames). |
| 2026-07-11 | Fix "typed shell command vanishes on Enter" — echo submitted input into the scrollback | ~10K (inline) | The dashboard cleared `InteractionController.inputLine` on Enter and `BuildScreen` only renders the *current* input line, so the typed command disappeared (IN/INS deliver input to the process without echoing, unlike a real tty). Fix: `SpectreDashboard.SubmitInput`/`SubmitStringInput` now call `EchoInput(text)` → `model.RecordOutput(hw.GetActiveProcess(), text)` (echo to the process the input is actually routed to, which is also what the Screen focuses), so the command persists above the fresh prompt. Covers both the live keyboard and the auto-script paths. Regression `SpectreDashboardTests.Screen_EchoesTypedInput_SoItStaysVisibleAfterEnter` (INS-loop program that never OUTs → the echoed line is the only Screen content; verified red without the fix; used `RenderSnapshot(console, 0)` as a render-only seam). 723 tests. |
| 2026-07-11 | Visualizer process **names** (panels showed bare numbers / "(none)" for forked/exec'd procs) | ~20K (inline) | The MLFQ/tree/queue/screen panels used `FriendlyName(row.Path)` = `NameForBase(programAddress)`, which is only populated at boot `LoadProcess` — so forked/exec'd processes (shell children, snake) had no name. Fix: resolve a process's program **from its `ProcessEntryFirstBlock`** (set by spawn + `fs_exec_core@2081`) via the FS dir tree. New `FsDiskView.NameByFirstBlock(snapshot)`, `BuddyHeapView.ProcessRow.FirstBlock`, and `PlainTextRenderer.ProcessLabel(row, disk)` = registered name → FS-file-by-firstblock → "pN" fallback; wired into all panels + the Screen header. Added `Process.DisplayName` (→ `namesByBase`) so `RunShell` labels the boot shell "shell". Snake→"snake", ls→"ls", etc. 5 `ProcessNamingTests`. 728 tests. **Diagnosed (via a headless shell→/bin/snake repro) the real "snake invisible" cause: snake renders & auto-focuses correctly, but the run loop paces ~1 user-instr per StepDelayMs(100ms) at High detail while a snake frame is ~1–2k user-instrs → minutes per frame (looks frozen).** |
| 2026-07-11 | Make full-screen programs (snake) visible/playable: frame-pacing + runtime speed control | ~30K (inline) | User chose **Both**. (1) **Frame-pacing** (`SpectreDashboard.ShouldFramePace` pure predicate + `RunFocusedFrame` burst): when the focused process is running and redrawing a whole screen (latest output is a multi-line canvas frame, or Ready-with-no-output priming the first frame), the run loop runs flat out until its next output/block/`FramePaceInstrCap`, so **one auto-tick = one frame** (paced by DelayMs → ~10fps@100ms, steerable) instead of 1 instr/tick. (2) **Speed control** (`InteractionController` speed ladder {0,2,5,10,25,50,100,200,400,800}ms + `+`/`-` keys, `DelayMs` property; status footer shows speed). Verified headless: snake advances exactly 1 frame/tick and a Down key steers it (survives to the bottom wall instead of dying at the right wall). 8 tests (6 `ShouldFramePace` predicate + 2 speed-control in `FrameHistoryTests`). 736 tests. |
| 2026-07-11 | Snake fixes: **place_food hang/pinned-food bug** + pacing consistency + slightly-slower default | ~35K (inline) | **Real correctness bug** ("died on the left, process didn't die"): `Programs.Snake()` `place_food` did `StR(RNG, EAX)`, but `StR(addr,src)` loads the address into **EAX first** (clobbering the value), so rng was pinned to the address constant → food never moved (always cell (0,2)) and eating it looped `place_food` forever (retrying the now-occupied fixed cell). Fix: hold the new rng in **R11** across the store and derive fx/fy from R11. Verified headless: food now moves (6,0)→(7,1), eating gives S:001, game terminates. Regression `SnakeTests.Snake_EatingFood_...` (deterministic: seed 12345 → food (6,0); steer up column 6 to eat; total-step budget catches a hang). **Pacing (`SpectreDashboard`):** (a) `ShouldFramePace` now gates on the focused process being **Ready** (not "some process running") so a just-terminated snake doesn't spin the burst — smooths the clock; (b) new `FramePaceExtraMs=55` fixed per-frame pause → snake ~6.5fps default (slightly slower than 10fps) and steadier (the fixed pause dominates the variable per-frame compute/paging time). `ShouldFramePace` predicate tests updated (signature dropped `hasRunningProcess`). 736 tests. |
| 2026-07-11 | Reader-doc sweep for the visualizer commit `e37ee37` (naming/focus/pacing/echo) | ~15K (inline) | `e37ee37` updated the internal CLAUDE.md files but **not** the reader docs, so they had drifted. `docs/Visualizer-Guide.md`: Screen panel (per-line scrollback + input echo + why — IN/INS don't echo), process **names** across panels, `+`/`-` speed ladder in the key table, rewritten **focus model** (Screen follows the OS foreground via SETFOCUS; Tab = manual override that lapses), new **run-pacing** paragraph + §7.4 `ShouldFramePace`/`RunFocusedFrame` internals, snake section re-paced ("responsive-but-slow" → frame-paced). `README.md`: added `+`/`-`, `d`, `Ctrl-C`/`Ctrl-Z` to the controls table + a frame-pacing note; **fixed a stale constant** (process table "192 bytes each" → 208, from the JC-E growth). `OSTests/CLAUDE.md`: 721/64 → **737/65** + a `ProcessNamingTests` row. Docs-only, 737 tests still green. |
| 2026-07-11 | Screen panel: one output entry per line (terminal scrollback) | ~6K (inline) | `BuildScreen`'s non-canvas branch joined the last `ScreenLines` outputs with `"  "` onto one row; now each entry (shell command echo + each OUT/OUTS) renders on its **own line**, so a shell session reads like a transcript. Test `SpectreDashboardTests.Screen_ShowsEachOutputOnItsOwnLine_NotJoined` (needs a short `DisplayName` so both outputs fit the tiny headless panel — a long name wraps and clips). 737 tests. |
